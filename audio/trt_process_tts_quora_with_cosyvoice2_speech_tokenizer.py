                ...
                with open(transcript_file, "rb") as file:
                    for byte_line in file:
                        ...
                        if len(files_batch) == batch_size:
                            speech_tokens = [tokenizer.extract_speech_token(x) for x in files_batch]
...
def write_webdataset(json_file_list, output_tar_prefix, num_processes, batch_size=16):
    ...
    with Pool(num_processes) as pool:
        pool.starmap(process_json_file, [(json_file, output_tar_prefix, idx % num_gpus, batch_size) for idx, json_file in enumerate(json_file_list)])
